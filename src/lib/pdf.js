...
function sealCompany(co) {
  if (co && co.sealImg) return '<img src="' + co.sealImg + '" class="seal-img" alt="社印"/>';
  return '<div class="seal">社印</div>';
}
function sealPerson(co) {
  if (co && co.personSealImg) return '<img src="' + co.personSealImg + '" class="seal-img" alt="担当印"/>';
  return '<div class="seal">担当</div>';
}
...
export function buildDeliveryHTML(doc, co) {
  ...
  const seals = "<div class=\"seals\">" + sealPerson(co) + "<div class=\"seal\">確認</div>" + sealCompany(co) + "</div>";
  return "<!DOCTYPE html><html lang=\"ja\"><head><meta charset=\"utf-8\"><title>納品書 " + no + "</title>" + css() + "</head><body>" +
    ...
    "<h1>納　品　書</h1>" +
    "<div class=\"hd\"><div><div class=\"to-name\">" + (doc.customer || "") + " 御中</div><div class=\"to-sub\">" + (doc.toAddr || "") + "</div></div>" +
    coBlock(co, date, no, "納品書") + "</div>" +
...
export function buildInvoiceHTML(doc, co) {
  ...
  const seals = "<div class=\"seals\">" + sealPerson(co) + sealCompany(co) + "</div>";
  ...
  return "<!DOCTYPE html><html lang=\"ja\"><head><meta charset=\"utf-8\"><title>請求書 " + no + "</title>" + css() + "</head><body>" +
    "<button class=\"pbtn\" onclick=\"window.print()\">🖨 印刷 / PDF保存</button>" +
    "<h1>請　求　書</h1>" +
    "<div class=\"hd\"><div><div class=\"to-name\">" + (doc.customer || "") + " 御中</div>" +
    "<div class=\"to-sub\">" + (doc.toAddr || "") + "</div>" + toContact + "</div>" +
    coBlock(co, date, no, "請求書") + "</div>" +
    "<div class=\"sbj\"><span class=\"sbj-l\">件名</span><span>" + (doc.subject || "") + "</span></div>" +
    ...
    "<p class=\"tnote\">※印は軽減税率対象です。</p>" +
    taxBlock(tx) +
    "<div class=\"ftr\"><div><div class=\"ftr-l\">入金期日</div><div>" + (doc.dueDate ? fd(doc.dueDate) : "") + "</div></div>" +
    "<div><div class=\"ftr-l\">振込先</div><div style=\"line-height:1.7\">" + bank + "</div></div></div>" +
    "<div class=\"note-box\"><div class=\"nlbl\">備考</div>" + (doc.note || "") + "</div>" +
    seals +
    "</body></html>";
}
...
export function buildReceiptHTML(doc, co) {
  ...
  const companySeal = co && co.sealImg
    ? "<img src=\"" + co.sealImg + "\" class=\"seal-img\" style=\"width:22mm;height:22mm\" alt=\"社印\"/>"
    : "<div class=\"seal\" style=\"width:22mm;height:22mm\">社印</div>";
  return "<!DOCTYPE html><html lang=\"ja\"><head><meta charset=\"utf-8\"><title>領収書 " + no + "</title>" + css() + "</head><body>" +
    ...
    "<div style=\"font-size:8.5pt;color:#555;margin-top:2mm\">（うち消費税等 ¥" + fm(tax) + "）</div></div>" +
    ...
    "<div class=\"seals\"><div class=\"seal\" style=\"width:22mm;height:22mm\">収入印紙</div>" + companySeal + "</div>" +
    "</body></html>";
}
