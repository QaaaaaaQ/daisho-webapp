...
function ItemsEditor({ items, products, onChange }) {
  const upd = (i, k, v) => {
    const next = items.map((it, j) => {
      ...
      if (k === "qty" || k === "price") updated.amount = Number(updated.qty||0) * Number(updated.price||0);
      return updated;
    });
    onChange(next);
  };
  ...
  const fill = (i, p) => {
    const updated = { ...items[i], name: p.name, unit: p.unit, price: p.price, origin: p.origin||"", taxRate: p.taxRate||8 };
    updated.amount = Number(updated.qty||0) * Number(updated.price||0);
    onChange(items.map((it,j)=>j===i?updated:it));
  };
  ...
  return <div>
      ...
      <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12, minWidth:780 }}>
        ...
        <tbody>
          {items.map((it,i)=><tr key={i} style={{ borderBottom:"1px solid #f0f0f0" }}>
            ...
            <td style={{ padding:"3px 4px" }}><input style={{ ...INP, width:55, fontSize:11, padding:"4px 5px" }} value={it.unit||""} onChange={(e)=>upd(i,"unit",e.target.value)} placeholder="kg"/></td>
            ...
            <td style={{ padding:"3px 4px" }}>
              <select style={{ ...SEL, width:60, fontSize:11, padding:"4px 5px" }} value={it.taxRate||8} onChange={(e)=>upd(i,"taxRate",Number(e.target.value))}>
            ...
            <td style={{ padding:"3px 4px" }}><button onClick={()=>remItem(i)} style={{ background:"none", border:"none", cursor:"pointer", color:"#9ca3af", fontSize:16, padding:"0 4px" }}>×</button></td>
          </tr>)}
        </tbody>
      </table>
    </div>
    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:8 }}>
      ...
      {items.length>0&&<div style={{ fontSize:12, color:"#6b7280" }}>
        小計: {fm(tx.sub)}円 ／ 消費税: {fm(tx.tax)}円 ／ <strong>合計: {fm(tx.total)}円</strong>
      </div>}
    </div>
  </div>;
}
...
function DirectDocForm({ co, products, customers, history, setHistory, setProducts, setCustomers, user, onClose }) {
  ...
  const fillCustomer = (name) => {
    ...
    upd("customer", name);
    if (c) { if(c.bank) upd("bank",c.bank); }
  };
  const tx = calcTax(f.items);
  ...
  return <Modal title={"📝 " + f.docType + "を直接作成"} onClose={onClose} maxW={900} tall>
    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginBottom:12 }}>
      ...
      <Field label="取引先">
        <select style={SEL} value={f.customer} onChange={(e)=>fillCustomer(e.target.value)}>
  ...
  return <div style={{ background:"#fff", border:"1.5px solid #e5e7eb", borderRadius:10, overflow:"hidden", marginTop:8, maxWidth:420, boxShadow:"0 2px 8px rgba(0,0,0,0.07)" }}>
    ...
    <div style={{ padding:"9px 13px" }}>
      ...
      {(doc.items||[]).slice(0,3).map((it,i)=><div key={i} style={{ display:"flex", justifyContent:"space-between", fontSize:12, padding:"2px 0", borderBottom:"1px solid #f3f4f6" }}>
        <span>{it.origin&&it.origin+" "}{it.name}{it.taxRate!==10?" ※":""} × {it.qty}{it.unit}</span>
...
function CustomersView({ customers, setCustomers }) {
  ...
  const save = async () => {
    ...
    await db.saveCustomer(f);
    const next = await db.getCustomers();
    setCustomers(next); setM(null);
  };
  const del = async (id) => {
    if (!confirm("削除しますか？")) return;
  ...
  return <div style={{ height:"100%", display:"flex", flexDirection:"column" }}>
    ...
    <div style={{ flex:1, overflowY:"auto" }}>
      <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
        <thead><tr style={{ background:N, color:"#fff" }}>{["コード","顧客名","住所","担当者","TEL",""].map((h,i)=><th key={i} style={{ padding:"7px 10px", fontWeight:400, textAlign:"left" }}>{h}</th>)}</tr></thead>
        <tbody>
          {customers.map((c,i)=><tr key={c.id} style={{ background:i%2===0?"#fff":"#f9fafb", borderBottom:"1px solid #f0f0f0" }}>
            ...
            <td style={{ padding:"7px 10px" }}><div style={{ display:"flex", gap:5 }}><Btn small onClick={()=>open(c)}>編集</Btn><Btn small variant="red" onClick={()=>del(c.id)}>削除</Btn></div></td>
          </tr>)}
