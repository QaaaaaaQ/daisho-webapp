...
- ユーザーへの質問や説明文は一切不要。JSONのみ返す`;
      ...
      const custSection = customers?.length > 0
        ? `\n\n【顧客マスター】取引先名は必ず以下から正式名を選んでください（略称・部分一致でもOK）:\n${(customers as string[]).join("、")}`
        : "";
      const prodSection = products?.length > 0
        ? `\n\n【商品マスター】品名が一致する場合は産地・単位・単価を以下から補完してください:\n${(products as any[]).slice(0, 80).map(p => `・${p.name}${p.origin ? `（${p.origin}）` : ""}${p.unit ? ` / ${p.unit}` : ""}${p.price ? ` / ${p.price}円` : ""}`).join("\n")}`
        : "";

      const raw = await callGemini(systemPrompt + custSection + prodSection, text);
